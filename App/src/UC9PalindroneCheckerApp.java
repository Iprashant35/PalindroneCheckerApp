/**
 * =====================================================
 * MAIN CLASS - UseCase9RecursivePalindrome
 * =====================================================
 *
 * Use Case 9: Recursive Palindrome Checker
 *
 * Description:
 * This class validates a palindrome using recursion.
 *
 * Characters are compared from the outer positions
 * moving inward using recursive calls.
 *
 * The recursion stops when:
 * - All characters are matched, or
 * - A mismatch is found.
 *
 * This use case demonstrates divide-and-conquer
 * logic using method recursion.
 *
 * NOTE: The original problem statement included an
 * image illustrating the code structure and comments.
 *
 * @author Developer
 * @version 9.0
 */
public class UC9PalindromeCheckerApp {

    /**
     * Application entry point for UCP.
     *
     */
    public static void main(String[] args) {

        String input = "madam";
        System.out.println("Input : " + input);

        boolean result = check(input, 0, input.length() - 1);

        System.out.println("Is Palindrome? : " + result);
    }

    /**
     * Recursively checks whether a string is palindrome.
     *
     * @param s     Input string
     * @param start Starting index
     * @param end   Ending index
     * @return true if palindrome, otherwise false
     */
    private static boolean check(String s, int start, int end) {

        // Base case: all characters matched or single/middle character reached
        if (start >= end) {
            return true;
        }

        // If mismatch found, not a palindrome
        if (s.charAt(start) != s.charAt(end)) {
            return false;
        }

        // Recursive call: move towards the center
        return check(s, start + 1, end - 1);
    }
}
